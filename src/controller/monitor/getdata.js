...
function exchangePromQL(startTime, endTime) {
  ...
  const unixStart = (start.getTime() ) / 1000;
...
class DataCollectController {
  ...
  async getCurrentInfo(ctx) {
    ...
    // console.log(mysqlconfig.table_name_database)
    ...
    if (!cpuInfo.data.result[0])
      res.data.push({ name: 'cpuInfo', msg: 'CPU数据获取失败' })
    else res.data.push({ name: 'cpuInfo', data: cpuInfo.data.result[0].value })
    ...
    if (!readInfo.data.result[0]) res.data.push({ name: 'currentDiskRead[30s] byte/s', data: '', msg: "无法获取此时的信息" })
    else res.data.push({ name: 'currentDiskRead[30s] byte/s', data: readInfo.data.result[0].value })

    qstr = PromQL.currentDiskWrite;
    qstr = qstr.replace(/NodeName/g, NodeName)
    const writeInfo = await query_api(uri_query + para + qstr)
    if (!writeInfo.data.result[0]) res.data.push({ name: 'currentDiskkWrite[30s] byte/s', data: '', msg: "无法获取此时的信息" })
    else res.data.push({ name: 'currentDiskWrite[30s] byte/s', data: writeInfo.data.result[0].value })
    ...
    if (!waitInfo.data.result[0]) res.data.push({ name: 'iowait ms/s', data: '', msg: "无法获取此时的信息" })
    else res.data.push({ name: 'iowait ms/s', data: waitInfo.data.result[0].value })
